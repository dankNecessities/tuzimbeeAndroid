...
export default function CategoryScreen({route, navigation}) {
  ...
  // TODO Load default items when screen is opened
  useEffect(() => {
    let tempArray = [];
    Utils.cutArray(searchRows, 2, tempArray);
    setResultArray(tempArray);
    ...
    <MainContainer>
      <Container
        ...
        <Input
          ...
          borderColor="#e0e0e0"
        />
        <GenericHeading align="center">{route.params.text}</GenericHeading>
        {/* FIXME Not necessary to use if here when using filtered API Call */}
        <ItemContainer>
          {resultArray.map((_, i) => {
            return (
              <RowContainer>
                {_.map((item, index) => {
                  if (item.category === route.params.text) {
                    return (
                      <Product
                        key={item.id}
                        title={item.title}
